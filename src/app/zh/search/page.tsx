...
export default async function SearchPageZh({
  ...
  if (q) {
    try {
      const docs = await getSearchDocuments('zh');
      initialHits = runSearch(createSearchIndex(docs), q, 20);
    } catch (error) {
      console.error('[SearchPageZh] server fallback search failed:', error);
    }
  }

  return <SearchPageClient locale="zh" initialQuery={q} initialHits={initialHits} />;
}
